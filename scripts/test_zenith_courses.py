"""Zenith course battery — 48 m/s (near terminal 51.6) (physics ceiling 44 m/s, MAX_ACCEL 40).
...
Usage:
    python scripts/test_zenith_courses.py
"""
...
ZENITH_COURSES = [
    ...
    (
        "zenith_slalom_50",
        [(np.array([32.*(i+1), 14.*(i%2), -3.0 if i%2==0 else -5.0]),
    ...
    (
        "zenith_altitude_straight_20",
        [(np.array([42.*(i+1), 0., -2.0 if i%2==0 else -6.0]), np.array([1., 0., 0.]))
         for i in range(20)],
        16152, 17152,
    ),
    ...
    (
        "zenith_helix_20",
        (
            _helix_outward(cx=0., cy=0., radius=26., z_start=-3.0, z_step=-0.5, n_gates=10, start_angle=0.)
            + _helix_outward(cx=0., cy=0., radius=24., z_start=-8.0, z_step=0.5, n_gates=10, start_angle=np.pi)
        ),
        16153, 17153,
    ),
    ...
    (
        "zenith_hypersonic_40",
        [(np.array([float(i * 46), 0., -3.0]), np.array([1., 0., 0.])) for i in range(1, 41)],
        16155, 17155,
    ),
    ...
    (
        "zenith_triple_helix_24",
        (
            _helix_outward(cx=0., cy=0., radius=24., z_start=-3.0, z_step=-0.5, n_gates=8, start_angle=0.)
            + _helix_outward(cx=45., cy=0., radius=24., z_start=-7.0, z_step=0.5, n_gates=8, start_angle=np.pi)
            + _helix_outward(cx=90., cy=0., radius=24., z_start=-3.0, z_step=-0.5, n_gates=8, start_angle=0.)
        ),
        16157, 17157,
    ),
]
...
def main() -> int:
    print(f"\nZENITH COURSE BATTERY — max_speed={_MAX_SPEED} m/s")
    print("=" * 65)
    results = []
    for name, defs, mp, vp in ZENITH_COURSES:
        defs_list = list(defs)
