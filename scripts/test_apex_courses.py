"""Apex course battery — 42 m/s (near terminal 44.7) (physics ceiling 44 m/s, MAX_ACCEL 40).
...
Usage:
    python scripts/test_apex_courses.py
"""
...
APEX_COURSES = [
    ...
    (
        "apex_slalom_50",
        [(np.array([26.*(i+1), 12.*(i%2), -3.0 if i%2==0 else -5.0]),
    ...
    (
        "apex_altitude_straight_20",
        [(np.array([34.*(i+1), 0., -2.0 if i%2==0 else -6.0]), np.array([1., 0., 0.]))
         for i in range(20)],
        16102, 17102,
    ),
    ...
    (
        "apex_helix_20",
        (
            _helix_outward(cx=0., cy=0., radius=22., z_start=-3.0, z_step=-0.5, n_gates=10, start_angle=0.)
            + _helix_outward(cx=0., cy=0., radius=20., z_start=-8.0, z_step=0.5, n_gates=10, start_angle=np.pi)
        ),
        16103, 17103,
    ),
    ...
    (
        "apex_hypersonic_40",
        [(np.array([float(i * 38), 0., -3.0]), np.array([1., 0., 0.])) for i in range(1, 41)],
        16105, 17105,
    ),
    ...
    (
        "apex_triple_helix_24",
        (
            _helix_outward(cx=0., cy=0., radius=20., z_start=-3.0, z_step=-0.5, n_gates=8, start_angle=0.)
            + _helix_outward(cx=45., cy=0., radius=20., z_start=-7.0, z_step=0.5, n_gates=8, start_angle=np.pi)
            + _helix_outward(cx=90., cy=0., radius=20., z_start=-3.0, z_step=-0.5, n_gates=8, start_angle=0.)
        ),
        16107, 17107,
    ),
]
...
def main() -> int:
    print(f"\nAPEX COURSE BATTERY — max_speed={_MAX_SPEED} m/s")
    print("=" * 65)
    results = []
    for name, defs, mp, vp in APEX_COURSES:
        defs_list = list(defs)
